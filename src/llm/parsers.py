...
class QuestionDecomposerOutputParser(BaseOutputParser):
    """Parses question decomposer outputs embedded in markdown code blocks containing JSON."""
    ...
    def parse(self, output: str) -> Any:
        ...
        """
        logging.debug(f"Parsing output with QuestionDecomposerOutputParser: {output}")
        if "```json" in output:
...
class SchemaCriticOutputParser(BaseOutputParser):
    """Parses question decomposer outputs embedded in markdown code blocks containing JSON."""
...
class RecomposeOutputParser(BaseOutputParser):
    """Parses output embedded in markdown code blocks containing SQL queries."""
    ...
    def parse(self, output: str) -> Dict[str, str]:
        ...
        Returns:
            Dict[str, str]: A dictionary with the SQL query.
        """
        logging.debug(f"Parsing output with RecomposeOutputParser: {output}")
        if "<FINAL_ANSWER>" in output and "</FINAL_ANSWER>" in output:
            output = output.split("<FINAL_ANSWER>")[1].split(
            "</FINAL_ANSWER>"
            )[0]
...
def get_parser(parser_name: str) -> BaseOutputParser:
    ...
    parser_configs = {
        ...
        "question_decomposer": lambda: JsonOutputParser(pydantic_object=QuestionDecomposerOutputParser),
        ...
        "schema_critic": lambda: JsonOutputParser(pydantic_object=SchemaCriticOutputParser),
        ...
        "recompose": RecomposeOutputParser(),
        "sql_decomposer": lambda: JsonOutputParser(pydantic_object=QuestionDecomposerOutputParser),
