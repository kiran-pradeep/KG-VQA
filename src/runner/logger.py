...
from runner.task import SubQuestion, Task
...
class Logger:
    ...
    def __new__(cls, question_id: str = None, result_directory: str = None):
        """
        ...
        with cls._lock:
            if question_id is not None:
                ...
                cls._instance._init(question_id, result_directory)
            else:
                if cls._instance is None:
                    raise ValueError("Logger instance has not been initialized.")
            return cls._instance

    def _init(self, question_id: str, result_directory: str):
        """
    ...
    def log(self, message: str, log_level: str = "info", task: Task = None):
        ...
        if log_method is None:
            raise ValueError(f"Invalid log level: {log_level}")
        if task is not None:
            log_method(f"{task.question_id}) {message}")
        else:
            log_method(message)
    ...
    def log_conversation(self, conversations: List[Dict[str, Any]]):
        ...
        with self.log_file_lock:
            log_file_path = self.result_directory / "logs" / f"{self.question_id}.log"
            log_file_path.parent.mkdir(parents=True, exist_ok=True)
            with log_file_path.open("a") as file:
                for conversation in conversations:
                    text = conversation["text"]
                    file.write(
                        f"############################## {conversation['from']} at step {conversation['step']} ##############################\n\n"
                    )
    ...
    def dump_history_to_file(self, execution_history: List[Dict[str, Any]]):
        ...
        """
        file_path = self.result_directory / f"{self.question_id}.json"
        file_path.parent.mkdir(parents=True, exist_ok=True)

        def subquestion_to_dict(sq):
            return {
                "id": sq.id,
                "question": sq.question,
                "answer": sq.answer,
                "paragraph_support_idx": sq.paragraph_support_idx,
            }

        # Apply this to the sub_questions list before dumping
        for entry in execution_history:
            if "qd_result" in entry and "sub_questions" in entry["qd_result"]:
                entry["qd_result"]["sub_questions"] = [
                    subquestion_to_dict(sq) if isinstance(sq, SubQuestion) else sq
                    for sq in entry["qd_result"]["sub_questions"]
                ]
