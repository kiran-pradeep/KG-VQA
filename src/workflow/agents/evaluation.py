...
class QDAccuracy(Tool):
    ...
    def _run(self, state: SystemState):
        ...
        try:
            ...
            actual_dict = state.task.qd_verdict.model_dump() if state.task.qd_verdict else None
                ...
                for key in actual_dict:
                    ...
                    try:
                        if predicted_dict[key] == actual_dict[key]:
                            ...
                            exec_res = 0
                            exec_err += f"{key}:{predicted_dict[key]}!={actual_dict[key]}; "
                    except KeyError:
                        print(f"KeyError: {key} not found in predicted_dict or actual_dict")    

            self.evaluation_results["final_result"] = {
                "exec_res": 0 if exec_res < 5 else 1,
                "exec_err": exec_err,
                "Question": state.task.question,
                "sub_questions": state.sub_questions if state.sub_questions else state.task.question_decomposition,
    ...
    #             predicted_sql = "--error--"
    #             Logger().log(
    #                 f"Node 'evaluation': {state.task.question_id}\n{type(e)}: {e}\n",
    #                 "error",
    #             )
    #             evaluation_result = {
    #                 "exec_res": "error",
